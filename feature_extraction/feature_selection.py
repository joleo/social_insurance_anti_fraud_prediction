...
def validate_clf(train, train_y, features, clf, score, v = False): #用于挑选分类器特征
    ...
    for index_train, index_eval in kf.split(train, train_y):
        X_train, X_test = train.iloc[index_train], train.iloc[index_eval]
        y_train, y_test = train_y[index_train], train_y[index_eval]
        print('train')
        Testtemp = []
    ...
    y_pred = clf.predict(X_test, num_iteration=gbm.best_iteration)
